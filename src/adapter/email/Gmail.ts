...
export class OutlookEmail {
    ...
    public async sync(){
        ...
        try{
            console.log("[1]: Đang kết nối tới Gmail...");
            ...
            try{
                ...
                if(uids && uids.length > 0){
                    const sortedUids = (uids as any).sort((a: number, b: number) => b - a);
                for (const uid of sortedUids){
                    console.log(`[4]: Tìm thấy ${(uids as any).length} mail mới.`);
                    ...
                    if (mailDate < oneHourAgo) {
                        console.log(`[Skip]: Mail UID ${uid} gửi lúc      
                        ${mailDate.toLocaleString()} (quá 1 tiếng), bỏ qua.`);
                        continue; // Nhảy sang mail tiếp theo
                    }
                    ...
                    if(mail.subject?.toLowerCase().includes('[mindx]')){
                        ...
                        await this.createTicket.execute({title: new TicketTitle(mail.subject), description: mail.text || "Empty", customerEmail: mail.from?.value[0]?.address || "unknow"});
                ...
                console.log(`[Gmail]: Không có mail mới.`)
            }
            }finally{
                lock.release();
            }
